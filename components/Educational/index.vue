<template>
  <main data-aos="fade-up">
    <div class="mb-5 flex flex-col items-center justify-center text-center">
      <img src="@/assets/icons/education.webp" alt="logo" class="w-16 mb-3 mx-auto" width="100%" height="100%"
        sizes="100%" />
      <h4 class="font-bold text-blue-600">Educational</h4>
    </div>

    <div class="sm:flex sm:justify-center">
      <div class="divSatu flex sm:flex-col sm:flex-end gap-4 mb-4 sm:mb-0 hidden sm:block">
        <article data-aos="flip-left" data-aos-easing="ease-out-cubic" data-aos-duration="500" class="hidden sm:block">
          <img src="@/assets/images/unila.webp" alt="unila" class="object-fit cover sm:w-[300px] rounded-2xl" width="100%"
            height="100%" sizes="100%" />
        </article>
        <br />
        <article data-aos="flip-left" data-aos-easing="ease-out-cubic" data-aos-duration="500" class="hidden sm:block">
          <img src="@/assets/images/sma.webp" alt="sma" class="object-fit cover sm:w-[300px] rounded-2xl" width="100%"
            height="100%" sizes="100%" />
        </article>
        <br />
        <article data-aos="flip-left" data-aos-easing="ease-out-cubic" data-aos-duration="500" class="hidden sm:block">
          <img rounded="2xl" src="@/assets/images/smp.webp" alt="unila" class="object-fit cover sm:w-[300px] rounded-2xl"
            width="100%" height="100%" sizes="100%" />
        </article>
      </div>
      <!-- <hr class="vertical text-black rotate-90 w-[600px] h-[2px] border-[4px] ml-8 mt-[18rem]" /> -->
      <div class="sm:flex sm:flex-start flex-col">
        <article
          class="p-2 lg:p-4 shadow-md rounded border-t-[6px] border-blue-900 md:border-none sm:w-[300px] sm:h-[180px] lg:w-[auto]"
          data-aos="fade-up-left">
          <h2 class="font-bold text-[13px] sm:text-xl text-center">
            University of Lampung
          </h2><br />
          <p class="text-[10px] sm:text-[18px] sm:font-medium">
            Informatics Engineering
          </p>
          <p class="text-[10px] sm:text-[18px] sm:font-medium">IPK 3.88</p>
          <p class="text-[10px] sm:text-[18px] sm:font-medium pb-2">
            2020 - Now
          </p>
        </article>
        <br />
        <article
          class="p-2 lg:p-4 shadow-md rounded border-t-[6px] border-blue-900 md:border-none sm:w-[300px] sm:h-[180px] lg:w-[auto]"
          data-aos="fade-up-left">
          <h2 class="font-bold text-[13px] sm:text-xl text-center">
            YP Unila Senior High School
          </h2><br />
          <p class="text-[10px] sm:text-[18px] sm:font-medium">
            Matematics and Natural Science
          </p>
          <p class="text-[10px] sm:text-[18px] sm:font-medium">
            Average Report (88/100)
          </p>
          <p class="text-[10px] sm:text-[18px] sm:font-medium pb-2">
            2017 - 2020
          </p>
        </article>
        <br />
        <article
          class="p-2 lg:p-4 shadow-md rounded border-t-[6px] border-blue-900 md:border-none sm:w-[300px] sm:h-[180px] lg:w-[auto]"
          data-aos="fade-up-left">
          <h2 class="font-bold text-[13px] sm:text-xl text-center">
            Junior High School 2 Penawartama
          </h2><br />
          <p class="text-[10px] sm:text-[18px] sm:font-medium">
            Natural Science
          </p>
          <p class="text-[10px] sm:text-[18px] sm:font-medium">
            General Champion
          </p>
          <p class="text-[10px] sm:text-[18px] sm:font-medium pb-2">
            2014 - 2017
          </p>
        </article>
      </div>
    </div>
  </main>
</template>


